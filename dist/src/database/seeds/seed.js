...
async function seed() {
    ...
    try {
        ...
        if (developerRole) {
            const dailyUpdatePermissions = createdPermissions.filter(p => p.name.includes('DAILY_UPDATES') || p.name.includes('CREATE') || p.name.includes('UPDATE') || p.name.includes('DELETE'));
