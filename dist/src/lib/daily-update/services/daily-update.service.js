...
const zoho_people_service_1 = require("./zoho-people.service");
let DailyUpdateService = class DailyUpdateService {
    constructor(dailyUpdateRepository, userService, projectService, zohoPeopleService) {
    ...
    async searchDailyUpdates(searchDto, currentUserId) {
        console.log(...oo_oo(`2195729967_149_4_149_89_4`, `🔍 Search request from user ${currentUserId} with criteria:`, searchDto));
        const hasFullPermission = await this.userService.hasPermission(currentUserId, 'VIEW_DAILY_UPDATES_FULL');
        const hasBasicPermission = await this.userService.hasPermission(currentUserId, 'VIEW_DAILY_UPDATES');
        console.log(...oo_oo(`2195729967_163_4_163_129_4`, `🔐 User permissions - VIEW_DAILY_UPDATES_FULL: ${hasFullPermission}, VIEW_DAILY_UPDATES: ${hasBasicPermission}`));
        if (!hasBasicPermission && !hasFullPermission) {
            console.log(...oo_oo(`2195729967_166_6_166_84_4`, `❌ User ${currentUserId} has no permission to view daily updates`));
            return {
                data: [],
                total: 0,
        ...
        console.log(...oo_oo(`2195729967_183_4_183_79_4`, `📋 User ${currentUserId} has access to projects:`, projectIds));
        if (projectIds.length === 0) {
            console.log(...oo_oo(`2195729967_187_6_187_69_4`, `❌ User ${currentUserId} has no projects assigned`));
            return {
                data: [],
                total: 0,
                page: searchDto.page || 1,
        ...
        if (searchDto.userId) {
            if (hasFullPermission) {
                ...
                console.log(...oo_oo(`2195729967_207_8_207_100_4`, `✅ User with VIEW_DAILY_UPDATES_FULL can filter by userId: ${searchDto.userId}`));
            }
            else {
                searchCriteria.userId = currentUserId;
                console.log(...oo_oo(`2195729967_212_8_212_133_4`, `⚠️ User with only VIEW_DAILY_UPDATES - ignoring requested userId ${searchDto.userId}, showing own updates only`));
            }
        }
        else if (!hasFullPermission) {
            searchCriteria.userId = currentUserId;
            console.log(...oo_oo(`2195729967_218_6_218_94_4`, `🔒 User without VIEW_DAILY_UPDATES_FULL - restricting to own updates only`));
        }
        if (searchDto.projectId) {
            if (!hasFullPermission && !projectIds.includes(searchDto.projectId)) {
                console.log(...oo_oo(`2195729967_226_8_226_112_4`, `❌ User ${currentUserId} tried to access project ${searchDto.projectId} without permission`));
                return {
                    data: [],
                    total: 0,
                    page: searchDto.page || 1,
            ...
            console.log(...oo_oo(`2195729967_238_6_238_69_4`, `✅ Project filter applied: ${searchDto.projectId}`));
        }
        else if (!hasFullPermission) {
            searchCriteria.projectId = projectIds;
            console.log(...oo_oo(`2195729967_243_6_243_67_4`, `🔒 Restricting to user's projects:`, projectIds));
        }
        if (searchDto.teamId) {
            searchCriteria.teamId = searchDto.teamId;
            console.log(...oo_oo(`2195729967_250_6_250_63_4`, `✅ Team filter applied: ${searchDto.teamId}`));
        }
        if (searchDto.status) {
            searchCriteria.status = searchDto.status;
            console.log(...oo_oo(`2195729967_255_6_255_65_4`, `✅ Status filter applied: ${searchDto.status}`));
        }
        if (searchDto.tickets) {
            searchCriteria.tickets = searchDto.tickets;
            console.log(...oo_oo(`2195729967_260_6_260_67_4`, `✅ Tickets filter applied: ${searchDto.tickets}`));
        }
        if (searchDto.startDate && searchDto.endDate) {
            ...
            console.log(...oo_oo(`2195729967_266_6_266_96_4`, `✅ Date range filter applied: ${searchDto.startDate} to ${searchDto.endDate}`));
        }
        console.log(...oo_oo(`2195729967_269_4_269_60_4`, `🔍 Final search criteria:`, searchCriteria));
        console.log(...oo_oo(`2195729967_270_4_270_125_4`, `🔐 Permission summary - hasFullPermission: ${hasFullPermission}, hasBasicPermission: ${hasBasicPermission}`));
        console.log(...oo_oo(`2195729967_271_4_271_64_4`, `📋 User project IDs: ${projectIds.join(', ')}`));
        const page = searchDto.page || 1;
        const limit = searchDto.limit || 10;
        const offset = (page - 1) * limit;
        console.log(...oo_oo(`2195729967_277_4_277_84_4`, `📄 Pagination - page: ${page}, limit: ${limit}, offset: ${offset}`));
        ...
        console.log(...oo_oo(`2195729967_285_4_285_78_4`, `📊 Raw data count: ${rawData.length}, Total count: ${total}`));
        if (rawData.length > 0) {
            console.log(...oo_oo(`2195729967_287_6_292_8_4`, `📝 Sample raw data:`, {
                id: rawData[0].id,
        ...
        if (hasFullPermission) {
            ...
            console.log(...oo_oo(`2195729967_303_6_303_111_4`, `👁️ VIEW_DAILY_UPDATES_FULL: Showing ${filteredData.length} updates across user's projects`));
        }
        else {
            filteredData = rawData.filter(item => item.userId === currentUserId);
            console.log(...oo_oo(`2195729967_309_6_309_113_4`, `👤 VIEW_DAILY_UPDATES: Showing ${filteredData.length} updates for user ${currentUserId} only`));
        }
        ...
        console.log(...oo_oo(`2195729967_342_4_342_108_4`, `✅ Search completed - Returning ${filteredData.length} results, page ${page}/${totalPages}`));
        return {
            data,
    ...
    async createWithZohoSync(zohoSyncDto) {
        try {
            console.log(...oo_oo(`2195729967_373_6_373_92_4`, `🚀 Creating daily update with Zoho sync for user: ${zohoSyncDto.userId}`));
            const user = await this.userService.findOne(zohoSyncDto.userId);
            if (!user) {
                throw new common_1.NotFoundException(`User with ID ${zohoSyncDto.userId} not found`);
            }
            const project = await this.projectService.findOne(zohoSyncDto.projectId);
            if (!project) {
                throw new common_1.NotFoundException(`Project with ID ${zohoSyncDto.projectId} not found`);
            }
            ...
            console.log(...oo_oo(`2195729967_412_6_412_77_4`, `✅ Daily update created with ID: ${createdDailyUpdate.id}`));
            const syncResponse = {
            ...
            if (zohoSyncDto.syncToZoho) {
                try {
                    console.log(...oo_oo(`2195729967_430_10_430_96_4`, `🔄 Starting Zoho People sync for daily update: ${createdDailyUpdate.id}`));
                    let zohoUserId = zohoSyncDto.zohoUserId;
                    let zohoProjectId = zohoSyncDto.zohoProjectId;
                    if (!zohoUserId) {
                        zohoUserId = user.employeeId || user.id;
                        console.log(...oo_oo(`2195729967_439_12_439_72_4`, `⚠️ Using fallback Zoho user ID: ${zohoUserId}`));
                    }
                    if (!zohoProjectId) {
                        zohoProjectId = project.code || project.id;
                        console.log(...oo_oo(`2195729967_445_12_445_78_4`, `⚠️ Using fallback Zoho project ID: ${zohoProjectId}`));
                    }
                    const logTimeEntries = this.prepareZohoLogTimeEntries(zohoSyncDto, totalHours);
                    if (logTimeEntries.length > 0) {
                        const zohoSyncResult = await this.zohoPeopleService.createMultipleLogTimeEntries(zohoUserId, zohoProjectId, zohoSyncDto.date, logTimeEntries, zohoSyncDto.zohoDraft || false);
                        ...
                        if (zohoSyncResult.success) {
                            ...
                            console.log(...oo_oo(`2195729967_471_14_471_98_4`, `✅ Zoho sync completed successfully: ${zohoSyncResult.entries} entries`));
                        }
                        else {
                            syncResponse.message = `Daily update created but Zoho sync had ${zohoSyncResult.errors.length} errors.`;
                            console.log(...oo_oo(`2195729967_474_14_474_103_4`, `⚠️ Zoho sync completed with errors: ${zohoSyncResult.errors.length} errors`));
                        }
                    }
                    else {
                        ...
                        console.log(...oo_oo(`2195729967_483_12_483_65_4`, `ℹ️ No log time entries to sync to Zoho`));
                    }
                }
                catch (zohoError) {
                    console.error(...oo_tx(`2195729967_486_10_486_57_11`, `❌ Zoho sync failed:`, zohoError));
                ...
                console.log(...oo_oo(`2195729967_495_8_495_71_4`, `ℹ️ Zoho sync not requested for this daily update`));
            }
            return syncResponse;
        }
        catch (error) {
            console.error(...oo_tx(`2195729967_500_6_500_77_11`, `❌ Failed to create daily update with Zoho sync:`, error));
            throw error;
        }
    }
    ...
    prepareZohoLogTimeEntries(dto, totalHours) {
        const entries = [];
    ...
    async testZohoConnection() {
        try {
            ...
            console.error(...oo_tx(`2195729967_590_6_590_58_11`, 'Zoho connection test failed:', error));
            return {
                success: false,
    ...
    async getZohoActivityTypes() {
        try {
            ...
            console.error(...oo_tx(`2195729967_605_6_605_64_11`, 'Failed to get Zoho activity types:', error));
    ...
    async getTimeTracking(timeTrackingDto, currentUserId) {
        ...
        console.log(...oo_oo(`2195729967_634_4_634_112_4`, 'Getting time tracking with criteria:', { projectId, teamId, startDate, endDate, filterTimeBy }));
        console.log(...oo_oo(`2195729967_635_4_635_50_4`, 'Current user ID:', currentUserId));
        ...
        console.log(...oo_oo(`2195729967_644_4_644_72_4`, 'Using date range:', { searchStartDate, searchEndDate }));
        const hasFullPermission = await this.userService.hasPermission(currentUserId, 'VIEW_DAILY_UPDATES_FULL');
        console.log(...oo_oo(`2195729967_652_4_652_82_4`, 'User has VIEW_DAILY_UPDATES_FULL permission:', hasFullPermission));
        let userProjectIds = [];
        userProjectIds = await this.userService.getUserProjectIds(currentUserId);
        console.log(...oo_oo(`2195729967_658_4_658_52_4`, 'User project IDs:', userProjectIds));
        if (userProjectIds.length === 0) {
            console.log(...oo_oo(`2195729967_662_6_662_50_4`, 'User has no projects assigned'));
            return [];
        }
        ...
        if (!projectId) {
            ...
            console.log(...oo_oo(`2195729967_673_6_673_66_4`, 'Restricting to user project:', searchProjectId));
        }
        if (projectId && !userProjectIds.includes(projectId)) {
            console.log(...oo_oo(`2195729967_678_6_678_67_4`, 'User does not have access to specified project'));
            return [];
        }
        ...
        console.log(...oo_oo(`2195729967_690_4_690_84_4`, 'Raw daily updates from DB:', JSON.stringify(dailyUpdates, null, 2)));
        if (!dailyUpdates || dailyUpdates.length === 0) {
            console.log(...oo_oo(`2195729967_693_6_693_66_4`, 'No daily updates found for the given criteria'));
            return [];
        }
        let filteredUpdates = dailyUpdates;
        if (hasFullPermission) {
            filteredUpdates = dailyUpdates.filter(update => userProjectIds.includes(update.projectId));
            console.log(...oo_oo(`2195729967_705_6_705_109_4`, 'Full permission: showing all updates under user projects, count:', filteredUpdates.length));
        }
        else {
            filteredUpdates = dailyUpdates.filter(update => update.userId === currentUserId);
            console.log(...oo_oo(`2195729967_711_6_711_98_4`, 'Limited permission: showing only user updates, count:', filteredUpdates.length));
        }
